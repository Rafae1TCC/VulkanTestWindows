...
namespace lve {
    ...
    LveSwapChain::~LveSwapChain() {
        ...
        // cleanup synchronization objects - now using image count
    ...
    VkResult LveSwapChain::acquireNextImage(uint32_t* imageIndex) {
        // Wait for the fence for the current frame
        vkWaitForFences(
            device.device(),
            1,
        ...
        VkResult result = vkAcquireNextImageKHR(
            ...
            swapChain,
            std::numeric_limits<uint64_t>::max(),
            imageAvailableSemaphores[currentFrame],  // Use semaphore for current frame
            VK_NULL_HANDLE,
            imageIndex);
    ...
    VkResult LveSwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
        // Check if this image is already in flight
        ...
        // Use the semaphore that was signaled when we acquired this image
        ...
        // Move to next frame, wrapping around
        currentFrame = (currentFrame + 1) % static_cast<uint32_t>(imageAvailableSemaphores.size());
    ...
    void LveSwapChain::createSyncObjects() {
        size_t imageCnt = imageCount();

        imageAvailableSemaphores.resize(imageCnt);
        renderFinishedSemaphores.resize(imageCnt);
        inFlightFences.resize(imageCnt);
        imagesInFlight.resize(imageCnt, VK_NULL_HANDLE);
